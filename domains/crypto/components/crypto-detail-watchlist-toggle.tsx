...
export default function CryptoDetailWatchlistToggle({
  ...
  const [animationState, setAnimationState] = useState<"select" | "unselect" | null>(null);
  ...
  const animationClassName =
    animationState === "select"
      ? "animate-watchlist-star-select"
      : animationState === "unselect"
        ? "animate-watchlist-star-unselect"
        : "";

  const handleToggle = () => {
    setAnimationState(isAdded ? "unselect" : "select");
  ...
  return (
    <div className="flex items-center justify-end gap-2">
      ...
      <button
        ...
        className={`inline-flex h-9 w-9 items-center justify-center rounded-full border border-[var(--card-border)] bg-[var(--card-bg)] transition-colors hover:border-[var(--card-hover-border)] hover:text-[var(--accent)] ${isAdded ? "text-[var(--accent)]" : "text-[var(--text-secondary)]"}`}
      >
        <svg
          viewBox="0 0 24 24"
          fill={isAdded ? "currentColor" : "none"}
          className={`h-5 w-5 ${animationClassName}`}
          xmlns="http://www.w3.org/2000/svg"
          aria-hidden="true"
          onAnimationEnd={() => setAnimationState(null)}
        >
          <path
